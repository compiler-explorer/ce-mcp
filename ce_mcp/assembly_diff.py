...
def generate_assembly_diff(asm1: str, asm2: str, label1: str = "Compiler 1", 
                          label2: str = "Compiler 2", context: int = 3) -> Dict[str, Any]:
...
def generate_side_by_side(lines1: List[str], lines2: List[str], 
                         max_width: int = 50) -> List[Tuple[str, str]]:
...
def generate_diff_summary(stats: Dict[str, Any], lines1: List[str], 
                         lines2: List[str]) -> str:
    ...
    if stats["unique_instructions_added"]:
        ...
        if len(stats['unique_instructions_added']) > 5:
            summary_parts.append(f"New instructions: {', '.join(new_insts)} (+{len(stats['unique_instructions_added'])-5} more)")
        else:
    ...
    if stats["unique_instructions_removed"]:
        ...
        if len(stats['unique_instructions_removed']) > 5:
            summary_parts.append(f"Removed instructions: {', '.join(removed_insts)} (+{len(stats['unique_instructions_removed'])-5} more)")
        else:
    ...
    if stats["unique_calls_added"]:
        ...
        if len(stats['unique_calls_added']) > 3:
            summary_parts.append(f"New function calls: {', '.join(new_calls)} (+{len(stats['unique_calls_added'])-3} more)")
        else:
    ...
    if stats["unique_calls_removed"]:
        ...
        if len(stats['unique_calls_removed']) > 3:
            summary_parts.append(f"Removed function calls: {', '.join(removed_calls)} (+{len(stats['unique_calls_removed'])-3} more)")
        else:
...
def compare_optimization_levels(source: str, compilers: List[Dict[str, str]], 
                               client: Any) -> Dict[str, Any]:
    """Compare assembly across different optimization levels."""
    results = {}
    
    for compiler_config in compilers:
        compiler_id = compiler_config["id"]
        options = compiler_config.get("options", "")
        
        # Extract optimization level
        opt_level = "O0"  # default
        ...
        # result = await client.compile(source, "c++", compiler_id, options, get_assembly=True)
        # results[opt_level] = result.get("asm", "")
